...
class FormulirGuruActivity : AppCompatActivity(), AdapterView.OnItemSelectedListener {
    ...
    private val guruRef = database.getReference("guru")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_formulir_guru)
        ...
        btnDaftar.setOnClickListener {
            ...
            if (hp.isEmpty()) {
                // Tampilkan pesan kesalahan jika kolom email kosong
                etHp.error = "Tanggal Lahir harus diisi"
                isDataValid = false
            }
            if (deskripsi.isEmpty()) {
                // Tampilkan pesan kesalahan jika kolom email kosong
                etDeskripsi.error = "Email harus diisi"
                isDataValid = false
            }
            if (alamat.isEmpty()) {
                // Tampilkan pesan kesalahan jika kolom email kosong
            ...
            if (isDataValid) {
                ...
                // Mengunggah gambar ke Firebase Storage
                    uploadImage(nama,nik,lahir,hp, deskripsi,alamat,provinsi,kabupaten,kecamatan,kelurahan)
                } else {
                // Jika gambar tidak dipilih, daftarkan guru tanpa gambar
                    registerGuru(nama,nik,lahir,hp, deskripsi,alamat,provinsi,kabupaten,kecamatan,kelurahan,"")
                }
            }
        }
...
//                Toast.makeText(this@FormulirGuruActivity,"yes",Toast.LENGTH_SHORT).show()
              val listResponse = response.body()?.provinsi
                ...
                spProvinsi.onItemSelectedListener=this@FormulirGuruActivity
                val adapter = ArrayAdapter(this@FormulirGuruActivity,android.R.layout.simple_spinner_dropdown_item,listNamaProv)
            ...
            override fun onFailure(call: Call<ResponseProvinsi>, t: Throwable) {
                ...
                Toast.makeText(this@FormulirGuruActivity,"Ini adalah eror"+"${t.message}",Toast.LENGTH_LONG).show()
            }
        })
    }
    ...
    private fun uploadImage(nama: String,nik: String,lahir: String,hp: String,  deskripsi: String, alamat: String, provinsi:String, kabupaten : String, kecamatan : String, kelurahan : String) {
        ...
        uploadTask.continueWithTask { task ->
            ...
            if (task.isSuccessful) {
                ...
                registerGuru(nama, nik,lahir,hp, deskripsi, alamat,provinsi,kabupaten,kecamatan,kelurahan,imageUrl)
            } else {
    ...
    private fun registerGuru(nama: String,nik:String,lahir:String,hp:String, deskripsi: String,alamat: String,provinsi: String,kabupaten: String,kecamatan: String,kelurahan: String, imageUrl: String) {
        ...
        if (currentUserId != null) {
            ...
            val guru = Guru(currentUserId, nama,nik,lahir,hp, deskripsi, alamat, provinsi,kabupaten, kecamatan, kelurahan, imageUrl)
            // Menyimpan objek Guru ke dalam Realtime Database
            guruRef.child(currentUserId).setValue(guru)
    ...
    private fun showKota(idProv: Int) {
        RetrofitClient.instance.getKota(idProv).enqueue(object:Callback<ResponseKota>{
            override fun onResponse(call: Call<ResponseKota>, response: Response<ResponseKota>) {
                ...
                spKabupaten.onItemSelectedListener=this@FormulirGuruActivity
                val adapter=ArrayAdapter(this@FormulirGuruActivity,android.R.layout.simple_spinner_dropdown_item,listNamaKab)
                spKabupaten.adapter=adapter
            }

            override fun onFailure(call: Call<ResponseKota>, t: Throwable) {
                Toast.makeText(this@FormulirGuruActivity,"${t.message}",Toast.LENGTH_LONG).show()
            }
        })
    }
    ...
    private fun showKecamatan(idKota: Int) {
        RetrofitClient.instance.getKecamatan(idKota).enqueue(object : Callback<ResponseKecamatan> {
            override fun onResponse(call: Call<ResponseKecamatan>, response: Response<ResponseKecamatan>) {
                ...
                spKecamatan.onItemSelectedListener = this@FormulirGuruActivity
                val adapter = ArrayAdapter(this@FormulirGuruActivity, android.R.layout.simple_spinner_dropdown_item, listNamaKec)
    ...
    private fun showKelurahan(idKecamatan: Int) {
        RetrofitClient.instance.getKelurahan(idKecamatan).enqueue(object :Callback<ResponseKelurahan>{
            override fun onResponse(
                ...
                spDesa.onItemSelectedListener=this@FormulirGuruActivity
                val adapter=ArrayAdapter(this@FormulirGuruActivity,android.R.layout.simple_spinner_dropdown_item,listNamaDesa)
