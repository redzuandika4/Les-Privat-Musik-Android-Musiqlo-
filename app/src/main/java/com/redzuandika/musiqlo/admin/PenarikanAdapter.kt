...
class PenarikanAdapter(
    private val listPenarikan: List<TransaksiTarik>,
    private val onItemClickListener: (String) -> Unit) :
    RecyclerView.Adapter<PenarikanAdapter.PenarikViewHolder>() {

    inner class PenarikViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        init {
            itemView.setOnClickListener {
                val kelasId = listPenarikan[adapterPosition].id ?: ""
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PenarikViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.item_status_transaksi, parent, false)
        return PenarikViewHolder(view)
    }

    override fun onBindViewHolder(holder: PenarikViewHolder, position: Int) {
        val penarikan = listPenarikan[position]
        holder.bind(penarikan)
        holder.itemView.setOnClickListener {
            val context = holder.itemView.context
            val intent = Intent(context,UpdatePenarikanActivity::class.java)
            intent.putExtra("idPenarikan",penarikan.id)
